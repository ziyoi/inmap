...
var (
	dxByLatGEOS = []float64{2428, 9709, 19407, 29081, 38719, 48310, 57843,
		67305, 76685, 85971, 95153, 104219, 113158, 121959, 130611, 139104, 147428, 155572,
		163527, 171282, 178829, 186158, 193260, 200127, 206749, 213120, 219232, 225076, 230646,
		235934, 240936, 245644, 250052, 254156, 257951, 261431, 264592, 267431, 269945, 272129,
		273982, 275501, 276685, 277531, 278039, 278209, 278039, 277531, 276685, 275501, 273982,
		272129, 269945, 267431, 264592, 261431, 257951, 254156, 250052, 245644, 240936, 235934,
		230646, 225076, 219232, 213120, 206749, 200127, 193260, 186158, 178829, 171282, 163527,
		155572, 147428, 139104, 130611, 121959, 113158, 104219, 95153, 85971, 76685, 67305,
		57843, 48310, 38719, 29081, 19407, 9709, 2428} // meters
	dyGEOS = 222592.48 // meters
)

func variableGridBasic(data map[string]dataHolder) {
	...
	imax := data["UPlusSpeed"].data.Shape[2]
	jmax := data["UPlusSpeed"].data.Shape[1]
	...
	id := 0
	for k := 0; k < kmax; k++ {
		log.Println("Creating basic grid for layer ", k)
		...
		// var cells []*gridCell
		//if k < config.HiResLayers {	// this is set to zero right now, so will skip the nesting
		...
		//}
		//sortCells(cells)
		cells := make([]*gridCell, 0, imax*jmax)
		for j := 0; j < jmax; j++ {
			for i := 0; i < imax; i++ {
				cell := new(gridCell)
				cell.Row = id
				id++
				cell.Dx = dxByLatGEOS[j]
				cell.Dy = dyGEOS
				getDataBasic(cell, data, i, j, k)
				getBasicNeighbors(cell, i, j, k, imax, jmax, kmax)
		...
		// writeJsonAndGob(cells, k)
		writeGobBasic(cells, k)
		log.Println("Created basic grid for layer ", k)
	}
}

func writeGobBasic(cells []*gridCell, k int) {
...
func getBasicNeighbors(cell *gridCell, i, j, k, imax, jmax, kmax int) {
	// neighbors west and east (includes connecting globe)
	if i > 0 {
		cell.IWest = []int{cell.Row - 1}
	} else {
		cell.IWest = []int{cell.Row + (imax - 1)}
	}
	if i < imax-1 {
		cell.IEast = []int{cell.Row + 1}
	} else {
		cell.IEast = []int{cell.Row - (imax - 1)}
	}
	// neighbors south and north, includes no neighbors at top/bottom level
	// as GEOS is a cylinder
	if j > 0 {
		cell.ISouth = []int{cell.Row - imax}
	}
	if j < jmax-1 {
		cell.INorth = []int{cell.Row + imax}
	}
	// neighbors above and below
	if k > 0 {
		cell.IBelow = []int{cell.Row - (imax * jmax)}
	}
	if k < kmax-1 {
		cell.IAbove = []int{cell.Row + (imax * jmax)}
	}
	cell.IGroundLevel = []int{j*imax + i}
}

func getDataBasic(cell *gridCell, data map[string]dataHolder, i, j, k int) {
	cell.Layer = k
	ncells := 1.0
